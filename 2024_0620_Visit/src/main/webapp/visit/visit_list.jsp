...
<script type="text/javascript">
	function del(f){
		let pwd = f.pwd.value; // 원래 비밀번호
		let c_pwd = f.c_pwd.value.trim(); // 내가 입력한 비번 
		// forEach에 있는 vo안에 pwd 
		if(pwd != c_pwd){
		...
		// 방법 1. 
		// f.action = "delete.do";
		// f.submit(); // 전송. form에 name이 있는 값을 다 넘긴다.
		
		// 방법 2. location전송(선택적으로 parameter 전송. 필요한 idx 만 보낸다. )
